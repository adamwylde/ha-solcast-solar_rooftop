    ...
    async def sites_data(self):
        ...
        try:
            ...
            resp: ClientResponse = await self.aiohttp_session.get(
                url=f"{self.options.host}/rooftop_sites/{self.options.rooftop_id}", params=params, ssl=False
            )
    ...
    async def http_data(self, dopast = False):
        ...
        try:
            ...
            for site in self._sites:
                ...
                ae = None
                if dopast:
                    ae = await self.fetch_data("estimated_actuals", 28, site=self.options.rooftop_id,  apikey=self.options.api_key)
                ...
                af = await self.fetch_data("forecasts", 168, site=self.options.rooftop_id,  apikey=self.options.api_key)
